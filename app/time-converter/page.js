...
export default function TimeConverter() {
  ...
  useEffect(() => {
    ...
    // IP-based Geolocation (Bypasses the browser permission prompt)
  ...
  const handleSearch = async (e) => {
    ...
    setIsSearching(true);
    try {
      // Free open-source geocoding API for global city search
  ...
  const addCity = async (cityData) => {
    ...
    try {
      ...
      console.error("Failed to fetch sun data for city", err);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-24">
      ...
      <main className="max-w-5xl mx-auto px-6 pt-12">
        <h1 className="text-3xl font-bold mb-8 text-slate-800">🌍 Date & Time Converter</h1>
        ...
        <div className="bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl shadow-lg p-8 mb-8 text-white relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end">
            <div>
              <p className="text-indigo-100 font-semibold mb-1 uppercase tracking-wider text-sm">
                {localSunData.city ? `Local Time (${localSunData.city})` : 'Your Local Time'}
              </p>
            ...
            <div className="mt-6 md:mt-0 bg-white/10 backdrop-blur-sm p-4 rounded-xl border border-white/20 flex gap-6">
              <div>
                ...
                <p className="font-semibold text-lg">
                  {localSunData.sunrise ? localSunData.sunrise.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--:--'}
                </p>
              </div>
              <div>
                <p className="text-xs text-indigo-200 uppercase tracking-wider mb-1">Sunset 🌙</p>
                <p className="font-semibold text-lg">
                  {localSunData.sunset ? localSunData.sunset.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '--:--'}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-12">
          <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 relative">
            ...
            <div className="relative z-20 min-w-[280px] w-full md:w-auto">
              <input 
                type="text" 
                placeholder={isSearching ? "Searching..." : "+ Search for a city..."}
                value={searchQuery}
                onChange={handleSearch}
                disabled={selectedCities.length >= 4}
                className="w-full p-3 pl-10 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
              />
                ...
                <ul className="absolute top-full left-0 w-full mt-2 bg-white border border-slate-200 shadow-xl rounded-xl overflow-hidden max-h-60 overflow-y-auto z-50">
                  {searchResults.map((result) => (
                    <li 
                      ...
                      className="p-3 hover:bg-slate-50 cursor-pointer border-b border-slate-100 last:border-0 flex flex-col transition-colors"
                    >
                      <span className="font-bold text-slate-800">{result.name}</span>
                      <span className="text-xs text-slate-500 font-medium mt-0.5">
                        {result.admin1 ? `${result.admin1}, ` : ''}{result.country} • {result.timezone}
                      </span>
                    </li>
                  ))}
                </ul>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {selectedCities.length === 0 && (
              <div className="col-span-1 md:col-span-2 text-center py-12 text-slate-400 border-2 border-dashed border-slate-200 rounded-xl">
                Search and add a city to compare time zones and daylight hours.
              </div>
            )}
          </div>
        </div>
