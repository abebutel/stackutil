      ...
      <main className="max-w-4xl mx-auto px-6 pt-12">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 md:p-10 mb-12 relative overflow-hidden flex flex-col md:flex-row items-center gap-10">
          ...
          <div className="w-full flex-1">
            <label className="block text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">{dict?.qr_app?.label || "Your Link or Text"}</label>
            <textarea 
              value={text} 
              onChange={(e) => setText(e.target.value)}
              placeholder={dict?.qr_app?.placeholder || "Enter URL, text, or Wi-Fi details..."}
          ...
          <div className="w-full flex-1 flex flex-col items-center justify-center bg-slate-50 border border-slate-200 rounded-2xl p-8">
            <div className="w-48 h-48 bg-white border border-slate-200 rounded-xl flex items-center justify-center mb-6 shadow-sm overflow-hidden p-2">
              {qrUrl ? <img src={qrUrl} alt="Generated QR Code" className="w-full h-full object-contain" /> : <span className="text-slate-400">{dict?.qr_app?.waiting || "Waiting for input..."}</span>}
            </div>
          ...
          <div className="space-y-8">
            <section>
              <h3 className="text-xl font-bold mb-3 text-slate-800">
                {dict?.qr_app?.how_to_title || "How to Create Your QR Code"}
              </h3>
              <p className="text-slate-600 leading-relaxed">
                {dict?.qr_app?.how_to_text || "Enter any website URL, plain text, or contact information into the input field above. The tool will instantly generate a high-resolution QR code that you can download and print for restaurant menus, business cards, or marketing materials."}
              </p>
            </section>

            <section>
              <h3 className="text-xl font-bold mb-3 text-slate-800">
                {dict?.qr_app?.best_practices_title || "Best Practices for Scanning"}
              </h3>
              <p className="text-slate-600 leading-relaxed">
                {dict?.qr_app?.best_practices_text || "For the best user experience, ensure your downloaded QR code maintains high contrast (such as black modules on a white background) and is printed large enough to be easily read by mobile device cameras."}
              </p>
            </section>

            <section className="bg-slate-50 p-6 rounded-xl border border-slate-200">
              <h3 className="text-xl font-bold mb-4 text-slate-800 flex items-center gap-2">
                <span>💡</span> {dict?.qr_app?.faq_title || "Frequently Asked Questions"}
              </h3>
              <h4 className="text-lg font-semibold text-slate-700 mb-2">
                {dict?.qr_app?.faq_q1 || "Do these QR codes ever expire?"}
              </h4>
              <p className="text-slate-600 leading-relaxed text-sm">
                {dict?.qr_app?.faq_a1 || "No! The QR codes generated by StackUtil are 'static'. This means the URL or text data is hard-coded directly into the geometric pattern. As long as the website URL you entered remains active, the QR code will work forever without any subscription fees."}
              </p>
            </section>
          </div>
        </article>
